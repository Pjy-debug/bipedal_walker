...
def main():
    ...
    crashes = []
    crash_num = 0
    #crashes = np.load('/mnt1/brx/dataset/nade_dqn_450_trans_new_35000.npy',allow_pickle=True)
    #crash_num = (crashes>0).sum()
    #crashes = list(crashes)
    
    for epoch in range(800001,max_epoch):  
        obs,_= env.reset()
        done = False
        reward = 0
        ...
        if epoch % 20 == 0:
            print(len(crashes))
            np.save(f'/mnt1/brx/Rocketdata/test_result/nade_07_{epoch}.npy',crashes)
            #episode_result = []
        if epoch % 2000 == 0:
            np.save(f'/mnt1/brx/Rocketdata/test_result/nade_episode_07_{epoch}.npy',episode_result)
            episode_result = []
    
    #np.save('/mnt1/brx/dataset/nade_450_trans.npy',crashes)
    np.save('/mnt1/brx/Rocketdata/test_result/nade_f5_wo1.npy',crashes)
