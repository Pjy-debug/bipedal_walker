...
try:
    ...
    raise error.DependencyNotInstalled(
        "{}. (HINT: you can install Box2D with 'pip install Box2D')".format(e)
    )from e
...
INITIAL_RANDOM = 450.0  # Set 1500 to make game harder
...
class LunarLander_ordinary_nade(gym.Env, EzPickle):
    """
    ## Description
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def __init__(
        ...
        wind_power: float = 15.0,
        turbulence_power: float = 1.5,
        criticality_model1 = None,
        criticality_model2 = None,
        #改成GPU
        device = "cuda"
    ):
        EzPickle.__init__(
        ...
        self.criticality_model1 = criticality_model1
        self.criticality_model2 = criticality_model2
        ...
        self.input_buffer = collections.deque(maxlen=10)
    ...
    def epsilon_greedy(self, pdf_before_epsilon, ndd_pdf, epsilon=0):
        # NDD epsilon greedy method
        #修改，原来（17，）无法broadcast
    ...
    def calcu_q(self,obs, cur_wind_mag, cur_torque_mag):
        input_list = []
        for item in self.input_buffer:
            input_list += item
            #print('pre_inputs:',len(inputs))
        stack_input = []
        
        for k in range(17):
            k_input = list(obs) + [cur_wind_mag, cur_torque_mag, k]
            new_k_input = input_list + k_input
            new_k_input = torch.tensor(new_k_input, dtype=torch.float32,device=self.device)
            stack_input.append(new_k_input)
        new_k_inputs = torch.stack(stack_input,dim=0).reshape(17,-1)
        #print(new_inputs.shape)
        #修改，移到GPU
        new_k_inputs = new_k_inputs.to(self.device)
        outputs = self.criticality_model1(new_k_inputs)
        outputs2,_,_,_  = self.criticality_model2(new_k_inputs,new_k_inputs,0.5)
        pred_cls2 = outputs2[:,-1]> 0.5
        return outputs, outputs2, stack_input
        
        # new_k_input =  list(obs) + [cur_wind_mag, cur_torque_mag]
        # new_k_input = torch.tensor(new_k_input, dtype=torch.float32,device=self.device)
        # new_k_input = new_k_input.reshape(1,-1)
        # outputs, _ = self.criticality_model1(new_k_input)
        return outputs
            
                
    def get_env_action(self, obs, cur_wind_mag, cur_torque_mag):
        criticality = []
        p_list = []
        all_inputs = []
        inputs = []
        ...
        """
        for k in range(17):
            cur_input = list(obs) + [cur_wind_mag, cur_torque_mag, k]
            cur_input = torch.tensor(cur_input, dtype=torch.float32,device=device)
            inputs = cur_input.reshape(1,-1)
        
            outputs,_,_,_ = self.criticality_model1(inputs,inputs,0.5)
            q = outputs[0][-1].item()
            #pred_cls = outputs[:,-1]>0.5
            #q = pred_cls[0].item()
        ...
        if len(self.input_buffer)<10:
            for k in range(17):
                cur_input = list(obs) + [cur_wind_mag, cur_torque_mag, k]
                cur_input = torch.tensor(cur_input, dtype=torch.float32,device=self.device)
                inputs = cur_input.reshape(1,-1)
            
                # outputs,_,_,_ = self.criticality_model2(inputs,inputs,0.5)
                # q = outputs[0][-1].item()
                # pred_cls = outputs[:,-1]>0.5
                # q = pred_cls[0].item()
            
                
                criticality.append(1)
                p_list.append(1)
                all_inputs.append(cur_input)
        
        else:
            
            outputs, outputs2, stack_inputs = self.calcu_q(obs,cur_wind_mag, cur_torque_mag)
            
            rewards = outputs["rewards"]
            for j in range(17):
                # # 2.2
                # if rewards[j,-1].item() < 2.2:
                #     q = 0
                # else:
                #     #q = pred_cls2[0].item() 
                #     r = outputs2[j][-1].item()
                ...
                # criticality.append(q)
                # p_list.append(1)1
                all_inputs.append(stack_inputs[j])
            
            
            #outputs,_,_ =  self.calcu_q(obs,cur_wind_mag, cur_torque_mag)
            #criticality = list(outputs[0].cpu().detach().numpy())
    
            criticality = list(outputs2[:,-1].cpu().detach().numpy())
            criticality = criticality[:len(criticality)//2]
            p_list = [1]*17
        
        #修改
        # 确保 criticality 是一个 NumPy 数组
        criticality = np.array(criticality)
        #if sum(criticality)>12.5  80:
        #if sum(criticality)>10  31:
        if np.sum(criticality)>0:
            criticality = np.array(criticality) / np.sum(criticality)
            p_list = np.array(p_list) / np.sum(p_list)
            pdf_array = self.epsilon_greedy(criticality, p_list) 
            pdf_array = np.array(pdf_array) / np.sum(pdf_array)
        else:
            p_list = np.array(p_list) / np.sum(p_list)
            pdf_array = p_list
        ...
        #print('weight:',weight)
        criticality_info = {'criticality':criticality, 'pdf_array':pdf_array, 'p_list':p_list, 'env_action':action_idx, 'all_inputs':all_inputs}
        return action_idx, criticality_info
    ...
    def step(self, action):
        ...
        if not (terminated or truncated):
            self.add_idx, criticality_info = self.get_env_action(np.array(state, dtype=np.float32), wind_mag, torque_mag)
            cur_input = list(state) + [wind_mag, torque_mag, self.add_idx]
            self.input_buffer.append(cur_input)
        ...
        return np.array(state, dtype=np.float32), reward, terminated, truncated, {'crash':crash, 'wind_idx':self.wind_idx, 'torque_idx':self.torque_idx,'wind_mag':wind_mag,'torque_mag':torque_mag,'criticality_info':criticality_info}
    ...
    def get_wind_idx(self):
        return self.wind_idx, self.torque_idx
    
    def get_add_idx(self):
        return self.add_idx
