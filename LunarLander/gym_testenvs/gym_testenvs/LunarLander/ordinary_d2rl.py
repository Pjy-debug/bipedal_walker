...
class LunarLander_ordinary_d2rl(gym.Env, EzPickle):
    """
    ## Description
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def epsilon_greedy(self, pdf_before_epsilon, ndd_pdf, epsilon=0.05):
    ...
    def compute_action_torch(self,observation):
        print(observation)
        obs = torch.reshape(torch.tensor(observation,device=self.device), (1,len(observation)))
        out = self.d2rl_model({"obs":obs},[torch.tensor([0.0]).to(self.device)],torch.tensor([1]).to(self.device))
        action = torch.argmax(out[0][0])
        return action

    def compute_action_torch_continuous(self,observation):
    ...
    def calcu_q(self, obs, cur_wind_mag, cur_torque_mag):
        ...
        outputs2 = outputs
        pred_cls2 = outputs2["rewards"][:,-1]> 0.31
    ...
    def get_env_action(self, obs, cur_wind_mag, cur_torque_mag):
        ...
        #print(list(obs))
        epsilon = self.compute_action_torch_continuous(list(obs))
        if sum(criticality)>0:
            criticality = np.array(criticality) / sum(criticality)
            p_list = np.array(p_list) / sum(p_list)
            pdf_array = self.epsilon_greedy(criticality, p_list, epsilon=epsilon) 
            #pdf_array = p_list
        else:
            p_list = np.array(p_list) / sum(p_list)
            pdf_array = p_list
        ...
        #print('weight:',weight)
        criticality_info = {'criticality':criticality, 'pdf_array':pdf_array, 'p_list':p_list, 'env_action':action_idx, 'all_inputs':all_inputs,'epsilon':epsilon}
