...
class LunarLander_nade(gym.Env, EzPickle):
    """
    ## Description
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def get_env_action(self, obs, cur_wind_idx, cur_torque_idx):
        ...
        if len(self.input_buffer)<11:
            for k in range(17):
                cur_wind_mag = math.tanh(math.sin(0.02 * (cur_wind_idx+k)) + math.sin(math.pi * 0.01 * (cur_wind_idx+k))) * self.wind_power
        
                cur_torque_mag = math.tanh(math.sin(0.02 * (cur_torque_idx+k)) + (math.sin(math.pi * 0.01 * (cur_torque_idx+k)))) * (self.turbulence_power)
            ...
            for k in range(17):
                ...
                cur_input = list(obs) + [cur_wind_mag, cur_torque_mag, k]
                new_input = inputs + cur_input
                new_input = torch.tensor(new_input, dtype=torch.float32,device=device)
                all_inputs.append(new_input)
                
            
            new_inputs = torch.stack(all_inputs,dim=0).reshape(17,-1)
            
            outputs = self.criticality_model1(new_inputs)
            outputs2, feats2, _ , _ = self.criticality_model2(new_inputs, new_inputs, 0.5)
    ...
    def step(self, action):
        ...
        if not (terminated or truncated):
            self.add_idx, cur_input, criticality_info = self.get_env_action(state, self.wind_idx, self.torque_idx)
