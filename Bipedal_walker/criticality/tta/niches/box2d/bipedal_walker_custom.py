...
class BipedalWalkerCustom(gym.Env):
    ...
    def _step(self, action):
        ...
        if pos[0] > (TERRAIN_LENGTH - TERRAIN_GRASS) * TERRAIN_STEP:
            ...
            finish = True
        return np.array(state), reward, done, {"finish": finish, "position": pos, "weight": np.prod(self.weights),'agent_i':self.agent_i,'pred_i':self.pred_i}
