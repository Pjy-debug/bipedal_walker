...
class CppnEnvParams:
    """
    Compositional pattern-producing networks, a general encoding mechanism
    """
    ...
    def __init__(self, cppn_config_path='config-cppn', genome_path=None):
        ...
        # print(f"y = {np.array([self.altitude_fn((xi, )) for xi in self.x])}")
    ...
    def get_mutated_params(self):
        is_valid = False
        while not is_valid:
            mutated: PrettyGenome = copy_genome(self.cppn_genome)
