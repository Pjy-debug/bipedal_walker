...
class Box2DNiche(Niche):
    ...
    def rollout(self, theta, random_state, render=False, eval=False):
        ...
        for env_config in self.env_configs.values():
            returns, lengths = simulate(
                self.model, seed=seed, train_mode=not eval, render_mode=render, num_episode=1, env_config_this_sim=env_config, env_params=self.env_params)
