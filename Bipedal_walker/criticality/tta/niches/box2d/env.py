...
from .bipedal_walker_test import BipedalWalkerTest
#from .bipedal_walker_adv import BipedalWalkerAdv
from .bipedal_walker_adv_new import BipedalWalkerAdv

def make_env(env_name, seed, render_mode=False, env_config=None, get_action=None):
    if env_name.startswith("BipedalWalkerCustom"):
        assert env_config is not None
        env = BipedalWalkerCustom(env_config, seed)
    elif env_name.startswith("BipedalWalkerTest"):
        assert env_config is not None
        env = BipedalWalkerTest(env_config, get_action, seed)
    elif env_name.startswith("BipedalWalkerAdv"):
        assert env_config is not None
        env = BipedalWalkerAdv(env_config, get_action, seed)
    else:
