...
class BipedalWalkerAdv(gym.Env):
    ...
    def _step_env(self, action, hardcore=True, sparse_IS=False, debug=False):
        # action : env_action
        print('env step!','terrain_state=',self.terrain_state)
        self.terrain.reverse()
        env_actions = []
        ...
        while self.current_i < self.pred_i or self.terrain_counter > 0:
            print('current_i=',self.current_i,'pred_i=',self.pred_i)
            env_action = {}
            ...
            if self.terrain_counter == 0:
                ...
                if self.terrain_state == self.GRASS and hardcore:
                    ...
                    if sparse_IS:
                        ...
                        q = p
                        p_orin *= p
                        print('terrain counter == 0 and terrain_state == grass, p=',p,'terrain_oneshot=',self.terrain_oneshot,'action=',action)
            ...
            if self.terrain_state == self.GRASS and not self.terrain_oneshot:
                ...
                if self.env_params is not None and self.env_params.altitude_fn is not None:
                    ...
                    if sparse_IS:
                        ...
                        q = p
                        p_orin *= p
                        print('GRASS=',self.GRASS,'terrain counter > 0 and terrain_state == grass, p=',p,'terrain_oneshot=',self.terrain_oneshot,'action=',action,'delta_v=',delta_v)
                        self.weights.append(p / q)
                    else:
                        delta_v = self.np_random.uniform(-1, 1) / SCALE
                        self.terrain_velocity += delta_v
                ...
                if sparse_IS:
                    ...
                    q = p
                    p_orin *= p
                    print('pit=',self.PIT,'terrain counter == 0 and terrain_state == git, p=', p,'terrain_oneshot=',self.terrain_oneshot,'action=',action,'pit_gap=',pit_gap)
                    self.weights.append(p / q)
                else:
                    pit_gap = 1.0 + self.np_random.uniform(*self.config.pit_gap)
                    self.weights.append(1)
                ...
                if sparse_IS:
                    ...
                    q = p
                    p_orin *= p
                    print('stump=',self.STUMP,'terrain counter == 0 and terrain_state == stump', 'p=',p,'terrain_oneshot=',self.terrain_oneshot,'action=',action,'width,height,float=',stump_width,stump_height,stump_float)
                    self.weights.append(p / q)
                else:
                    stump_width = self.np_random.integers(*self.config.stump_width)
        ...
        return env_actions, self.terrain_poly[init_i-1:self.current_i-1], init_i, self.current_i, cur_step_poly,p_orin
    ...
    def step(self,env_action=None):
        return self._step(env_action=None)

    def _step(self,env_action = None,agent_action=None):
        ...
        if env_action is not None:
            self.new_env_actions, self.new_terrain_poly, self.cur_start, self.cur_end, self.cur_step_poly,p_orin = self._step_env(env_action, sparse_IS=False)
            self.env_actions += self.new_env_actions
            self.p_orin.append(p_orin)
        
        """
        if self.env_act:
            cur_action, cur_weight = calculate_criticality(
                                                           self.terrain_state,
                                                           self.state,
                                                           self.pos, 
                                                           self.terrain, 
                                                           ...
                                                           self.config, 
                                                           self.env_actions)
            self.new_env_actions, self.new_terrain_poly, self.cur_start, self.cur_end, self.cur_step_poly = self._step_env(cur_action, sparse_IS=True)
            self.env_actions += self.new_env_actions
        ...
        return np.array(state), reward, terminated, not finish, info, self.p_orin
