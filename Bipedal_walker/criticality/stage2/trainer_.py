...
class Trainer:
    ...
    def train(self, epoch):
        ...
        for i, data1 in enumerate(self.train_pos_loader):
            ...
            outputs, feats1, feats2 = self.model(inputs)
            outputs_pre, feats1_pre, feats2_pre = self.pre_model(inputs)
            ...
            """
            # 双路并行，pre_model重点学习正样本，model重点学习负样本
            mix_feats = weight * feats + (1-weight) * feats
            # 需要把feats传回模型得到outputs和outputs_pre
            loss = weight * self.criterion1(outputs, labels_cls) + (1-weight) * self.criterion1(outputs_pre, labels_cls)
            """
            """
            # 串行学习
            ...
            # 串行学习，知识蒸馏
            loss  =  self.criterion1(outputs, labels_cls) + weight * self.kdLoss(feats2,feats2_pre)
