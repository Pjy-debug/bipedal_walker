...
class BipedalWalkerTest(gym.Env):
    metadata = {
        ...
        while self.current_i < self.pred_i or self.terrain_counter > 0:
            ...
            if self.terrain_state == self.GRASS and not self.terrain_oneshot:
                ...
                if sparse_IS:
                    ...
                    q = p
                    # print('terrain counter == 0 and terrain_state == git, p=', p)
                    self.weights.append(p / q)
                else:
                    pit_gap = 1.0 + self.np_random.uniform(*self.config.pit_gap)
                    self.weights.append(1)
                ...
                if sparse_IS:
                    ...
                    p = 1
                    q = p
                    # print('terrain counter == 0 and terrain_state == stump', 'p=',p)
                    self.weights.append(p / q)
                else:
                    stump_width = self.np_random.integers(*self.config.stump_width)
    ...
    def step(self, env_action):
        return self._step(env_action)

    def _step(self, env_action, agent_action=None):
        ...
        if env_action is not None:
            self.new_env_actions, self.new_terrain_poly, self.cur_start, self.cur_end, self.cur_step_poly = self._step_env(env_action, sparse_IS=True)
